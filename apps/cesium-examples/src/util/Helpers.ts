 ...
 * 2. 使用注释替换 import 语句以保持行号对齐，使编辑器中的行号与实际执行行号一致。
 * 3. 在末尾注入一行移除 `body.sandcastle-loading` 类的代码，
 *    使 bucket.html 中的 `#loadingOverlay` 隐藏、`#toolbar` 显示。
 *
...
export function embedInSandcastleTemplate(code: string, addExtraLine: boolean) {
  // 移除用户代码中的 Cesium import 语句
  // 因为 bucket.html 已通过 <script> 标签加载了 Cesium 全局对象
  // 使用注释替换以保持行号对齐
  ...
  return `${addExtraLine ? "\n" : ""}${processedCode}
// 用户代码执行完毕，隐藏 loading overlay 并显示 toolbar
