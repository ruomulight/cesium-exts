import { type BridgeToApp } from "./IframeBridge";
...
function simpleStringify(obj: object) {
  ...
  if (obj.constructor.name !== "Object") {
    ...
    const prototypeProperties = Object.entries(Object.getOwnPropertyDescriptors(Reflect.getPrototypeOf(obj)));
  ...
  const className = obj.constructor.name !== "Object" ? `${obj.constructor.name} ` : "";
...
function functionString(func: Function, signatureOnly?: boolean) {
  ...
  if (signatureOnly) {
    ...
    const functionSigMatch = functionAsString.toString().match(signaturePattern);
    return functionSigMatch ? `${functionSigMatch[0].replace("function", "ƒ")} {...}` : "ƒ () {...}";
  }
